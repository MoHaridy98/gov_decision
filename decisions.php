...
function projectList()
{
    $user_role = $_SESSION['user_role'];
    require 'connection.php';
    $sql = "SELECT * FROM `decision` where archive = 0 AND entry_type = '$user_role'";
    ...
    while ($row = mysqli_fetch_array($list)) {       
        ...
        <td>$row[6]</td>
        <td>$row[7]</td>
        <td><a target=\"_blank\" href=\"$filepath\" onClick='EXPORT'>$row[8]</a></td>
        ...
        <input type='submit' style='margin: 0 1px;' name='action' class='btn btn-primary' value='عرض'/>
        <input type='submit' style='margin: 0 1px;' name='action' onClick='deleteHandler(event)' class='btn btn-danger'value='ارشيف'/></form></td>
    </tr>";
    }
    $conn->close();
    return $options;
...
<body>
    <div id="wrapper">
        ...
        <div id="page-wrapper">
            <div class="row">
                <div class="col-lg-6">
                    <h1 class="page-header">القرارات</h1>
                </div>
                <div class="col-lg-6" style="direction: ltr;">
                    <a class='page-header btn btn-primary' href='new-decision.php'><i class='fa fa-edit fa-fw'></i> اضافة قرار</a>
                </div>
            </div>
            <!-- /.row -->
            <div class="row">
    ...
    <script>
        ...
        function deleteHandler(event) {
            var sure = confirm('هل انت متاكد من ارساله للارشيف؟');
            if (sure == false) {
